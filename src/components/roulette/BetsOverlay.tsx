...
import { BetType, SimpleValue, ColumnValue, SixainValue, CarreValueHight, CarreValueLow, TransversaleValue } from './betPositions'

interface Bet {
  type: BetType
  value: number | SimpleValue | ColumnValue | SixainValue | CarreValueHight | CarreValueLow | TransversaleValue
  amount: number
}
  ...
  return (
    <div className="grid grid-cols-14 grid-rows-5 gap-1 h-full">
      <div className="row-span-3 grid grid-rows-2 gap-1">
        ...
        <div className="w-full h-full relative">
          {bets.find(b => b.type === 'number' && b.value === 0) &&
            <BetMarker amount={bets.find(b => b.type === 'number' && b.value === 0)!.amount} />
          }
        </div>
        <div className="w-full h-full relative">
          {bets.find(b => b.type === 'number' && b.value === 37) &&
            <BetMarker amount={bets.find(b => b.type === 'number' && b.value === 37)!.amount} />
          }
        </div>
      </div>
      ...
      <div className="col-span-12 row-span-3 grid grid-rows-3 gap-1">
          ...
          {[3, 6, 9, 12, 15, 18, 21, 24, 27, 30, 33, 36].map(num => (
            <div key={num} className="w-full h-full relative">
              {bets.find(b => b.type === 'number' && b.value === num) &&
                <BetMarker amount={bets.find(b => b.type === 'number' && b.value === num)!.amount} />
              }
            </div>
          ))}
        </div>
        ...
        {/* Sixains */}
        {bets.filter(bet => bet.type === 'sixain').map(bet => (
          <div key={bet.value} className="absolute" style={{
        ...
        {bets.filter(bet => bet.type === 'carre' && isCarreHigh(bet.value)).map(bet => (
          <div key={bet.value} className="absolute" style={{
        ...
        {/* Transversales */}
        {bets.filter(bet => bet.type === 'transversale').map(bet => (
          <div key={bet.value} className="absolute" style={{
      ...
      <div className="row-span-3 col-start-14 grid grid-rows-3 gap-1">
        <div className="w-full h-full relative">
          {bets.find(b => b.type === 'column' && b.value === 'C3') &&
            <BetMarker amount={bets.find(b => b.type === 'column' && b.value === 'C3')!.amount} />
          }
        </div>
        <div className="w-full h-full relative">
          {bets.find(b => b.type === 'column' && b.value === 'C2') &&
            <BetMarker amount={bets.find(b => b.type === 'column' && b.value === 'C2')!.amount} />
          }
        </div>
        <div className="w-full h-full relative">
          {bets.find(b => b.type === 'column' && b.value === 'C1') &&
            <BetMarker amount={bets.find(b => b.type === 'column' && b.value === 'C1')!.amount} />
          }
        </div>
      </div>
        ...
        {[1, 2, 3].map(dozen => (
          <div key={dozen} className="w-full h-full relative">
            {bets.find(b => b.type === 'dozen' && b.value === dozen) &&
              <BetMarker amount={bets.find(b => b.type === 'dozen' && b.value === dozen)!.amount} />
            }
          </div>
        ))}
      </div>
        ...
        {['1-18', 'PAIR', 'ROUGE', 'NOIR', 'IMPAIR', '19-36'].map(value => (
          <div key={value} className="w-full h-full relative">
            {bets.find(b => b.type === 'simple' && b.value === value) &&
              <BetMarker amount={bets.find(b => b.type === 'simple' && b.value === value)!.amount} />
            }
          </div>
        ))}
      </div>
...
const isCarreHigh = (value: any): value is CarreValueHight => {
  return ['1-2-4-5', '4-5-7-8', '7-8-10-11', '10-11-13-14', '13-14-16-17',
    '16-17-19-20', '19-20-22-23', '22-23-25-26', '25-26-28-29',
    '28-29-31-32', '31-32-34-35'].includes(value)
}
