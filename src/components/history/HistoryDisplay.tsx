...
const HistoryDisplay: FC = () => {
  ...
  const renderNumber = (number: number) => {
    ...
    const bgColor = number === 0 || number === 37 ? 'bg-roulette-green' : isRed ? 'bg-roulette-red' : 'bg-roulette-black'

    return (
      <div className={`${bgColor} w-10 h-10 flex items-center justify-center text-white border rounded-full`}>
        {number === 37 ? '00' : number}
      </div>
    )
  }
    ...
    <div className="h-48 overflow-y-auto scrollbar-thin scrollbar-track-transparent scrollbar-thumb-roulette-gold/80">
      <div className="grid grid-cols-12 gap-1">
        {[...history].reverse().map((spin) => (
          <div key={spin.timestamp}>
            {renderNumber(spin.number)}
          </div>
        ))}
      </div>
    </div>
